...
export const saveNewsletter = async (req, res, next) => {
    try {
        ...
        user.countries = [...new Set([...(user.countries || []), ...(data.countries || [])])];
        user.categories = [...new Set([...(user.categories || []), ...(data.categories || [])])];
        user.lastInsightsSent = data.lastInsightsSent || user.lastInsightsSent;
        user.dailyInsightsEnabled = data.dailyInsightsEnabled || user.dailyInsightsEnabled;
